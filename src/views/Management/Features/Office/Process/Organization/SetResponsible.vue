<template>
  <div class="upaTeacherInfo" v-loading="loading">
    ...
    <div class="pagehead">
      <h1 v-if="$route.query.form == 'sponsor'">发起人设置</h1>
      <h1 v-else-if="$route.query.form == 'department'">部门负责人设置</h1>
      <h1 v-if="$route.query.form == 'course'">部门课程负责人设置</h1>
    </div>
...
export default {
  data(){
    return{
      ...
      department: this.$route.query.dep,
  ...
  methods:{
    ...
    getData(v){
      ...
      requestAjax({
        ...
        data:{
          type: "sel_teacher_organization",
          campus: this.$store.state.userCampus,
          department: this.$route.query.dep,
          id: this.$route.query.ognId,
          form: this.$route.query.form,
        },
        async: true,
        success:(res)=>{
      ...
      requestAjax({
        ...
        success:(res)=>{
          ...
          if(res){
            ...
            let content = this.form == 'sponsor' ? "设置发起人" : this.form == 'department' ? "设置部门负责人" : this.form == 'course' ? "设置课程负责人" : null;

            // 日志写入
            let obj = {
              content: content + JSON.stringify(this.$route.query.ognId),
              type: "修改记录",
              model: "组织架构模块",
              ip: sessionStorage.getItem('ip'),
              user: this.$store.state.userId,
