<template>
  <div class="subpage"
    ...
    <div class="add_record">
      <div class="pagehead">
        <h1>{{ getDate(new Date()) }} 填写实习处督查</h1>
      </div>
          ...
          <div class="item-table">
            <el-table
              ...
              <el-table-column
                ...
                sortable>
                <template v-slot="scope">
                  <el-input type="number" placeholder="实到人数" v-model="scope.row.actually_num" size="mini"></el-input>
                </template>
              </el-table-column>
              <el-table-column
                prop="discipline"
                ...
                sortable>
                <template v-slot="scope">
                  <el-input type="number" placeholder="分数" min="0" max="10" v-model="scope.row.discipline" size="mini"></el-input>
                </template>
              </el-table-column>
              <el-table-column
                prop="safety"
                ...
                sortable>
                <template v-slot="scope">
                  <el-input type="number" placeholder="分数" min="0" max="10" v-model="scope.row.safety" size="mini"></el-input>
                </template>
              </el-table-column>
              <el-table-column
                prop="health"
        ...
        success:(res)=>{
          ...
          for(let i in res){
            ...
            requestAjax({
              ...
              data:{
                action: "get",
                col: "skill_id,skill_name,student_num",
                request: JSON.stringify({
                  campus: this.$store.state.userCampus,
                  department: depId,
                }),
              ...
              success:(res2)=>{
                ...
                for(let i in data){
                  data[i].teacher = "";
                  data[i].should_num = data[i].student_num;
                  data[i].actually_num = "";
                  data[i].discipline = "";
                  data[i].safety = "";
                  data[i].health = "";
                  data[i].inspection = "";
                  data[i].other = "";
                  data[i].addDate = new Date().setHours(0,0,0,0);
                  data[i].semester = this.$store.state.semester;
                  data[i].campus = this.$store.state.userCampus;
                  data[i].school = this.$store.state.userSchool;
                  data[i].created_user = this.$store.state.userId;
                }
                res[i]['child'] = data;
              },
              async: false,
      ...
      requestAjax({
        ...
        success:(res)=>{
            ...
            for(let r=0;r<data[i]['child'].length;r++){
              if(!data[i]['child'][r]){
                errarr.push({
                  ...
                  skill: arr.filter(item=>item.department_id == res.data[i]['department_id'])[0]['child'][r].skill_name,
                });
              }
            }
          }
          if(errarr.length <= 0){
            ...
            // 日志写入
            let obj = {
              content: "大赛办督查填写 学期：" + this.$store.state.semester + " 日期：" + new Date().getTime(),
              type: "添加记录",
              model: "大赛办督查",
            ...
            for(let i in errarr){
              console.log(errarr[i].department + " 的 " + errarr[i].skill + " 登记有误！");
            }
            // 日志写入
            let obj = {
              content: "大赛办督查填写 学期：" + this.$store.state.semester + " 日期：" + new Date().getTime(),
